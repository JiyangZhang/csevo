...
class AlphaFilter(AbstractFilter):
    ...
    def process_data_concurrent(self, proj_list: Path):
        ...
        with ThreadPoolExecutor(8) as executor:
            futures = [executor.submit(self.process_data, Macros.repos_results_dir/proj) for proj in projects]
    ...
    def process_data(self, project_dir):
        try:
            ...
            method_project_evo = []
            for year in AlphaFilter.YEARS[:-1]:
                ...
                filtered_method_ids = AlphaFilter.alpha_filter(new_method_ids, curr_method_ids, method_data)
            ...
            IOUtils.dump(output_dir/"method-project-MN-filtered.json", IOUtils.jsonfy(method_project_evo), IOUtils.Format.json)
            return
        except:
    ...
    @staticmethod
    def alpha_filter(new_method_ids: List[int], curr_method_ids: List[int], method_data) -> List[int]:
        ...
        for method_id in new_method_ids:
            ...
            for curr_method_id in curr_method_ids:
                ...
                if new_method["name"] == curr_method["name"] and new_method["params"] == curr_method["params"] and \
                        new_method["class_name"] == curr_method["class_name"]:
